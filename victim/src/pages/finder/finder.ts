...
export class FinderPage extends BasePage {
  ...
  policeId: any;
  location: any = [];
  police: FirebaseListObservable<any>;
  assignedOffice: any;
  ...
  constructor(private platform: Platform,
    ...
    this.cases.subscribe(data => {
      ...
      this.police.subscribe(data => {
        let officer = data;
        console.log(officer);
        
        this.location.push({latitude: this.casesFiled[0].location.lat, longitude: this.casesFiled[0].location.long, type: this.casesFiled[0].type});
        if (this.policeId) {
          this.assignedOffice = officer[this.policeId];
          this.location.push({latitude: this.assignedOffice.lat, longitude: this.assignedOffice.long, type: "police"});
        }
